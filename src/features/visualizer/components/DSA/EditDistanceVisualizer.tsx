...
import { 
  ...
  Type, Database, Trophy, Cpu, Plus, Minus, RefreshCcw
} from "lucide-react";
...
interface DPStep {
  ...
  step: string;
  activeLine: number; 
  decision: "INSERT" | "DELETE" | "REPLACE" | "MATCH" | "NONE";
  dependencies: [number, number][]; 
  logs: string[];
}

export default function EditDistanceVisualizer({ speed = 800 }: { speed?: number }) {
  ...
  const history = useMemo(() => {
    ...
    for (let i = 0; i <= m; i++) {
        dp[i][0] = i;
        record(`Base case: i=${i}, j=0. Cost to convert prefix to empty string is ${i}.`, "INIT_BASE", i, 0, 0, "NONE", []);
    }
    for (let j = 0; j <= n; j++) {
        dp[0][j] = j;
        record(`Base case: i=0, j=${j}. Cost to convert empty string to prefix is ${j}.`, "INIT_BASE", 0, j, 0, "NONE", []);
    }
    ...
    for (let i = 1; i <= m; i++) {
      for (let j = 1; j <= n; j++) {
        ...
        if (s1[i-1] === s2[j-1]) {
          dp[i][j] = dp[i-1][j-1];
          addLog(`Characters match! No operation needed. Inherit diagonal: ${dp[i][j]}.`);
          record(`Match found: '${s1[i-1]}'. Copy diagonal cost.`, "COMMIT_MATCH", i, j, 2, "MATCH", [[i-1, j-1]]);
        } else {
          ...
          let dec: DPStep['decision'] = "REPLACE";
          let dep: [number, number] = [i-1, j-1];
          if (dp[i][j] === 1 + insert) { dec = "INSERT"; dep = [i, j-1]; }
          else if (dp[i][j] === 1 + del) { dec = "DELETE"; dep = [i-1, j]; }

          addLog(`Mismatch. Taking Min(Insert: ${insert}, Delete: ${del}, Replace: ${replace}) + 1 = ${dp[i][j]}.`);
          record(`Mismatch. Best operation: ${dec}.`, "COMMIT_OP", i, j, 3, dec, [dep]);
        }
      }
    }

    addLog(`Optimal Edit Distance: ${dp[m][n]}.`);
    record(`DP Complete. Minimum operations: ${dp[m][n]}.`, "COMPLETE", m, n, -1, "NONE", []);
  ...
  const generateNewStrings = () => {
    ...
    setS1(getRandomStr(5));
    setS2(getRandomStr(4));
    setCurrentIndex(0);
    setIsPlaying(false);
  };
      ...
      <div className="p-4 md:p-8 bg-[var(--card)] rounded-[2.5rem] shadow-2xl overflow-hidden relative flex flex-col">
        ...
        <div className="relative z-10 flex flex-wrap items-center justify-between mb-8 p-4 bg-[var(--card)] border border-[var(--border)] rounded-2xl shadow-sm gap-4">
          <div className="flex items-center gap-3">
             <button onClick={generateNewStrings} className="p-2.5 bg-[var(--card)] hover:bg-[var(--accent)] border border-[var(--border)] rounded-xl text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-all cursor-pointer"><RotateCcw size={14}/></button>
          </div>
          <button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8 relative p-3 md:p-6 bg-muted/30 rounded-[2rem]  overflow-hidden shadow-inner flex flex-col items-center">
                                    ...
                                    return (
                                        <div key={`${r}-${c}`} className="w-10 h-10 flex items-center justify-center relative">
                                            <motion.div
                                                initial={false}
                                                animate={{ 
                                                    scale: isCurrent ? 1.15 : isDep ? 1.1 : 1,
                                                    backgroundColor: isCurrent ? `${MANIM_COLORS.blue}20` : 
                                                                     isDep ? `${MANIM_COLORS.green}15` : "transparent",
                                                    borderColor: isCurrent ? MANIM_COLORS.blue : 
                                                                 isDep ? MANIM_COLORS.green : "var(--border)",
                                            ...
                                            >
                                                <span className={isCurrent ? "text-[var(--viz-cyan)]" : isDep ? "text-[var(--viz-deep-purple)]" : "text-muted-foreground"}>{val}</span>
                                            </motion.div>
                ...
                <div className="mt-6 flex flex-wrap justify-center gap-4 w-full">
                    <div className={`p-3 min-w-[120px] rounded-xl border transition-all ${currentStep.decision === "MATCH" ? "bg-[var(--viz-deep-purple)]/10 border-[var(--viz-deep-purple)] shadow-lg" : "bg-card/50 opacity-40"}`}>
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-[8px] font-black uppercase text-[var(--viz-deep-purple)] tracking-widest">Match</span>
                            <Trophy size={10} className="text-[var(--viz-deep-purple)]" />
                        </div>
                        <div className="text-[10px] font-mono text-muted-foreground">DP[i-1][j-1]</div>
                    </div>
                    
                    <div className={`p-3 min-w-[120px] rounded-xl border transition-all ${currentStep.decision === "REPLACE" ? "bg-[var(--viz-amber)]/10 border-[var(--viz-amber)] shadow-lg" : "bg-card/50 opacity-40"}`}>
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-[8px] font-black uppercase text-[var(--viz-amber)] tracking-widest">Replace</span>
                            <RefreshCcw size={10} className="text-[var(--viz-amber)]" />
                        </div>
                        <div className="text-[10px] font-mono text-muted-foreground">1 + DP[i-1][j-1]</div>
                    </div>

                    <div className={`p-3 min-w-[120px] rounded-xl border transition-all ${currentStep.decision === "INSERT" ? "bg-[var(--viz-cyan)]/10 border-[var(--viz-cyan)] shadow-lg" : "bg-card/50 opacity-40"}`}>
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-[8px] font-black uppercase text-[var(--viz-cyan)] tracking-widest">Insert</span>
                            <Plus size={10} className="text-[var(--viz-cyan)]" />
                        </div>
                        <div className="text-[10px] font-mono text-muted-foreground">1 + DP[i][j-1]</div>
                    </div>

                    <div className={`p-3 min-w-[120px] rounded-xl border transition-all ${currentStep.decision === "DELETE" ? "bg-[var(--viz-rose)]/10 border-[var(--viz-rose)] shadow-lg" : "bg-card/50 opacity-40"}`}>
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-[8px] font-black uppercase text-[var(--viz-rose)] tracking-widest">Delete</span>
                            <Minus size={10} className="text-[var(--viz-rose)]" />
                        </div>
                        <div className="text-[10px] font-mono text-muted-foreground">1 + DP[i-1][j]</div>
                    </div>
                </div>
            </div>
                ...
                <div className="p-3 md:p-6 bg-muted/20  rounded-[2rem] flex-1 min-h-[350px] md:min-h-[200px] w-full overflow-x-auto overflow-y-hidden touch-pan-x no-scrollbar">
                    ...
                    <div className="space-y-2 font-mono text-[9px]">
                        ...
                        <div className={`p-2 rounded-lg border transition-all ${currentStep.activeLine === 3 ? "bg-[var(--viz-rose)]/10 border-[var(--viz-rose)] text-[var(--viz-rose)]" : "border-transparent text-muted-foreground/40"}`}>
                            3. Else: 1 + Min(Insert, Delete, Replace)
                        </div>
                    </div>
                </div>
            </div>
