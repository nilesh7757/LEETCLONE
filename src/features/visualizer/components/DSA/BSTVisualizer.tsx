...
export default function BSTVisualizer({ speed = 800 }: { speed?: number }) {
  ...
  const handleSearch = () => {
    const val = inputValue.trim() !== "" ? parseInt(inputValue) : Math.floor(Math.random() * 89) + 10;
    recordOperation('SEARCH', val);
    setInputValue("");
  };

  const handleDelete = () => {
    const val = inputValue.trim() !== "" ? parseInt(inputValue) : Math.floor(Math.random() * 89) + 10;
    recordOperation('DELETE', val);
    setInputValue("");
  };
    ...
    <div className="flex flex-col gap-4 select-none font-sans w-full">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-[var(--card)] border border-[var(--border)] rounded-2xl shadow-sm">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={handleSearch} className="flex items-center gap-1.5 px-3 py-2 bg-[var(--viz-lime)]/10 hover:bg-[var(--viz-lime)]/20 rounded-xl text-[var(--viz-lime)] transition-all text-[10px] font-bold uppercase cursor-pointer" title="Search"><Search size={12}/> Search</button>
          <button onClick={handleDelete} className="flex items-center gap-1.5 px-3 py-2 bg-[var(--viz-rose)]/10 hover:bg-[var(--viz-rose)]/20 rounded-xl text-[var(--viz-rose)] transition-all text-[10px] font-bold uppercase cursor-pointer" title="Delete"><Trash2 size={12}/> Delete</button>
