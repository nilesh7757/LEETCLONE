      ...
      <div className="w-full max-w-5xl relative z-10 flex-1 flex flex-col">
        ...
        {gameState === "START" && (
          ...
          >
            <div className="text-center space-y-4 mb-10 max-w-2xl">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-xs font-black tracking-[0.25em] uppercase mb-2">
                <Terminal size={14} /> System Sandbox V2.0
              </div>
            ...
            <div className="w-full space-y-4 mb-12">
              <h3 className="text-xs font-black uppercase tracking-[0.3em] text-gray-500 border-b border-gray-800 pb-2">Select Arena Sector</h3>
                ...
                {TOPICS.map((topic) => {
                  ...
                  return (
                    <div
                      ...
                      className={`relative p-5 rounded-2xl bg-zinc-900/40 border cursor-pointer transition-all duration-300 group hover:scale-[1.02] flex flex-col justify-between h-40 ${
                        isSelected 
                    ...
                    >
                      <div>
                        <div className="flex justify-between items-start mb-3">
                          <div className={`p-2.5 rounded-xl bg-gradient-to-br ${topic.color} text-white shadow-lg`}>
                            <Icon size={20} />
                          </div>
                          {isSelected && (
                            <span className="text-[10px] bg-cyan-500/20 border border-cyan-500/30 px-2 py-0.5 rounded font-black text-cyan-400 tracking-wider">
                              ACTIVE
                            </span>
                          )}
                        </div>
                        <h4 className="text-base font-black text-white group-hover:text-cyan-400 transition-colors">{topic.title}</h4>
                        <p className="text-xs text-gray-400 line-clamp-2 mt-1 font-medium">{topic.desc}</p>
                      </div>
                    </div>
                  );
                })}
        ...
        {gameState === "PLAYING" && (
          <div className="flex-1 flex flex-col gap-6 py-4">
            {/* STATS HEADER */}
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 p-4 rounded-2xl bg-zinc-900/40 border border-zinc-800 backdrop-blur-md">
              <div className="flex flex-col items-center justify-center p-2 border-r border-zinc-800/50">
                <span className="text-[10px] font-black uppercase tracking-widest text-gray-500 flex items-center gap-1">
                  <Trophy size={12} className="text-amber-500" /> Score
                </span>
                <span className="text-2xl font-black font-mono text-white mt-1">{score}</span>
              </div>

              <div className="flex flex-col items-center justify-center p-2 md:border-r border-zinc-800/50">
                <span className="text-[10px] font-black uppercase tracking-widest text-gray-500 flex items-center gap-1">
                  <Zap size={12} className="text-cyan-400" /> Streak
                </span>
              ...
              <div className="flex flex-col items-center justify-center p-2 border-r border-zinc-800/50">
                <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Tier Level</span>
                <span className="text-2xl font-black font-mono text-white mt-1">0{level}</span>
              </div>

              <div className="flex flex-col items-center justify-center p-2 md:border-r border-zinc-800/50">
                <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Security Lives</span>
                <div className="flex gap-1 mt-2">
                  {Array.from({ length: maxLives }).map((_, idx) => (
                    <Heart
                      key={idx}
                      size={18}
                      className={idx < lives ? "text-rose-500 fill-rose-500 filter drop-shadow-[0_0_5px_rgba(244,63,94,0.5)]" : "text-zinc-800"}
                    />
                  ))}
                </div>
              </div>
              ...
              <div className="col-span-2 md:col-span-1 flex flex-col items-center justify-center p-2">
                <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Difficulty</span>
                <span className={`text-xs font-black tracking-widest uppercase mt-2 px-3 py-1 rounded-full ${
            ...
            {loadingObstacle ? (
              ...
              >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
                  ...
                  <div className="lg:col-span-7 flex flex-col p-6 rounded-3xl bg-zinc-900/20 border border-zinc-800 justify-between">
                    <div>
                      {/* Topic Tags */}
                      <div className="flex flex-wrap gap-2 mb-4">
                        <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded bg-zinc-800 text-zinc-300 border border-zinc-700">
                          {currentObstacle.topic}
                        </span>
                        <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded bg-cyan-950/40 text-cyan-400 border border-cyan-900/50">
                          {currentObstacle.subtopic}
                        </span>
                      </div>

                      {/* Question */}
                      <h2 className="text-xl font-bold leading-relaxed mb-6 text-white font-sans">
                        {currentObstacle.question}
                      </h2>

                      {/* Optional Code Snippet */}
                      {currentObstacle.codeBlock && (
                        <div className="relative rounded-2xl overflow-hidden border border-zinc-800/80 bg-[#0c0c0e] p-5 font-mono text-xs text-cyan-300/90 mb-6 leading-relaxed max-w-full overflow-x-auto shadow-inner">
                    ...
                    <div>
                      <AnimatePresence>
                        {showHint && (
                          <motion.div
                            ...
                            className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/20 text-amber-300 text-xs mb-4 leading-relaxed flex gap-2 items-start"
                          >
                            <AlertCircle size={16} className="text-amber-400 shrink-0 mt-0.5" />
                            <div>
                              <strong className="uppercase font-black text-[9px] tracking-wider text-amber-500 block mb-1">Decryption Cue:</strong>
                      ...
                      {!showHint && !showExplanation && (
                        <button
                          ...
                          className="text-[10px] font-black text-gray-500 hover:text-amber-400 uppercase tracking-widest flex items-center gap-1.5 transition-colors"
                        >
                          <HelpCircle size={12} /> Decrypt Hint (-10 Score Penalty if used)
                        </button>
                      )}
                    </div>
                  </div>
                    ...
                    {currentObstacle.options.map((option, idx) => {
                      ...
                      if (showResult) {
                        if (isCorrect) {
                          ...
                          iconElement = <Check size={16} className="text-emerald-400 shrink-0" />;
                        } else if (isSelected) {
                          cardStyle = "border-rose-500 bg-rose-500/5 text-rose-300 shadow-[0_0_15px_rgba(244,63,94,0.1)]";
                          iconElement = <X size={16} className="text-rose-400 shrink-0" />;
                        } else {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full p-5 rounded-2xl border text-left text-sm font-medium transition-all duration-300 flex items-center justify-between gap-4 cursor-pointer hover:scale-[1.01] ${cardStyle}`}
                        >
