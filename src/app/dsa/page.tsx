...
export default function DSAPage() {
  ...
  return (
    <div 
      ...
      className="w-full h-screen overflow-hidden bg-[#020202] text-[#e1e1e1] flex flex-col font-sans"
    >
      ...
      <header className="h-14 border-b border-white/5 bg-[#050505] flex items-center justify-between px-6 shrink-0 z-50 shadow-2xl">
         <div className="flex items-center gap-6">
            <div className="flex items-center gap-3">
               ...
               <span className="hidden sm:inline text-[11px] font-black uppercase tracking-[0.3em] text-white">Neuro.Flow</span>
            </div>
            ...
            <div className="flex items-center gap-3 text-[10px] font-mono text-[#52525b]">
               ...
               <span className="text-[#a1a1aa]">{activeGroup.toUpperCase()}</span>
               <ChevronRight size={10} />
               <span className="text-white font-bold truncate max-w-[120px] md:max-w-none">{selectedCategory.title}</span>
            </div>
         </div>

         <div className="flex items-center gap-6">
            <div className="hidden md:block relative group">
               <div className="absolute left-3 top-1/2 -translate-y-1/2 text-[#52525b] group-focus-within:text-[#3b82f6] transition-colors">
                  <Search size={14} />
               </div>
               <input 
                  type="text"
                  ...
                  className="bg-white/5 border border-white/5 rounded-full py-1.5 pl-10 pr-4 text-[11px] w-48 lg:w-64 focus:outline-none focus:ring-2 focus:ring-[#3b82f6]/20 transition-all font-mono"
               />
            </div>
            <button 
               onClick={() => setIsMobileMenuOpen(true)}
               className="lg:hidden p-2 hover:bg-white/5 rounded-lg text-[#a1a1aa]"
            >
               <Menu size={20} />
            </button>
         </div>
      ...
      <main className="flex-1 flex min-h-0 relative">
         ...
         <aside className="hidden lg:flex w-16 border-r border-white/5 bg-[#050505] flex flex-col items-center py-6 gap-6 shrink-0 z-40">
            <button 
               onClick={() => setIsSidebarVisible(!isSidebarVisible)}
               className={`p-3 rounded-xl transition-all hover:bg-white/5 ${isSidebarVisible ? "text-[#52525b]" : "text-[#3b82f6] bg-[#3b82f6]/10"}`}
            ...
            {groups.map(g => (
               <button
                  ...
                  className={`p-3 rounded-xl transition-all relative group ${
                     activeGroup === g.id ? "bg-[#3b82f6]/10 text-[#3b82f6]" : "text-[#52525b] hover:text-[#a1a1aa] hover:bg-white/5"
                  }`}
               >
                  {g.icon}
                  {activeGroup === g.id && (
                  ...
                  <div className="absolute left-full ml-4 px-3 py-1 bg-[#111] border border-white/10 rounded text-[9px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50">
                     {g.title}
                  </div>
               </button>
            ))}
         ...
         <AnimatePresence initial={false}>
            {(isSidebarVisible || isMobileMenuOpen) && (
               <motion.aside 
                  ...
                  className={`border-r border-white/5 bg-[#080808] flex flex-col shrink-0 z-[60] lg:z-30 overflow-hidden ${isMobileMenuOpen ? "fixed inset-0" : "relative"}`}
               >
                  <div className={`${isMobileMenuOpen ? "w-full" : "w-80"} flex flex-col h-full`}>
                     <div className="p-6 border-b border-white/5 flex items-center justify-between shrink-0">
                        <div className="flex items-center gap-3">
                           {isMobileMenuOpen && (
                              <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 -ml-2 hover:bg-white/5 rounded-lg text-[#52525b]">
                                 <ChevronRight className="rotate-180" size={18} />
                              </button>
                           )}
                           <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-[#52525b]">Protocol Index</h3>
                        </div>
                        ...
                        <div className="p-4 border-b border-white/5 flex gap-2 overflow-x-auto no-scrollbar shrink-0">
                           {groups.map(g => (
                              <button 
                                 key={g.id} 
                                 onClick={() => setActiveGroup(g.id)}
                                 className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest whitespace-nowrap transition-all ${activeGroup === g.id ? "bg-[#3b82f6] text-white" : "bg-white/5 text-[#52525b]"}`}
                              >
                                 {g.title}
                              </button>
                           ))}
         ...
         <section className="flex-1 flex flex-col min-w-0 bg-[#020202] relative overflow-hidden">
            ...
            {/* Stage Header */}
            <div className="h-14 flex items-center justify-between px-4 md:px-8 bg-transparent border-b border-white/5 shrink-0 z-20 overflow-x-auto no-scrollbar">
               <div className="flex items-center gap-4 shrink-0">
                  <div className="flex items-center gap-2">
                     <div className="w-1.5 h-1.5 rounded-full bg-[#22c55e] shadow-[0_0_8px_#22c55e]" />
                     <span className="text-[9px] font-black text-[#52525b] uppercase tracking-[0.3em] whitespace-nowrap">Neural Sync: Active</span>
                  </div>
                  <div className="h-3 w-px bg-white/10 hidden sm:block" />
                  <div className="hidden sm:flex items-center gap-2">
                     <Activity size={12} className="text-[#3b82f6]" />
                     <span className="text-[9px] font-black text-[#52525b] uppercase tracking-[0.3em] whitespace-nowrap">Latency: 12ms</span>
                  </div>
               </div>
